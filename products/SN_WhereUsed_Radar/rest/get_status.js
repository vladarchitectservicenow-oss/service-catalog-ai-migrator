// WhereUsed Radar — GET /status (read-only reporting endpoint)
// Copyright (C) 2026 Vladimir Kapustin
// SPDX-License-Identifier: AGPL-3.0
//
// Read-only endpoint. Dispatches on query parameters:
//   ?report=<scan_id>      → ranked impact report
//   ?workbench=<scan_id>   → remediation workbench (findings + suggested fix)
//   ?graph=<scan_id>       → dependency graph projection (nodes + edges)
//   ?markdown=<scan_id>    → Markdown export of the impact report
...
(function process(request, response) {
    try {
        ...
        if (q.report) {
            result.report = report.buildImpactReport(q.report);
        } else if (q.workbench) {
            result.workbench = report.buildRemediationWorkbench(q.workbench);
        } else if (q.graph) {
            result.graph = report.buildDependencyGraph(q.graph);
        } else if (q.markdown) {
            ...
            response.setBody(JSON.stringify({
                ...
                valid_params: ['report', 'workbench', 'graph', 'markdown']
            }));
            return;
        }
